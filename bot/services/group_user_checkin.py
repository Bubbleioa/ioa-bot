...
async def _handle_check_in(user: GroupUser) -> str:
    ...
    message = random.choice((f"{SELFNAME}最喜欢你啦w", "谢谢，你是个好人！", "对了，来喝杯茶吗？"))
    await user.update(
    ...
    logger.info(
        f"(USER {user.user_qq}, GROUP {user.belonging_group}) CHECKED IN successfully. score: {new_impression:.2f} (+{impression_added:.2f})."
    )
...
async def group_user_check(user_qq: int, group: int) -> str:
    ...
    return "好感度：{:.2f}\n历史签到数：{}\n上次签到日期：{}".format(
        user.impression,
        user.checkin_count,
        user.checkin_time_last.strftime("%Y-%m-%d")
